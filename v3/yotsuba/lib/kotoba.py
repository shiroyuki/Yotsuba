    ...
    *name* is the name of the node.
    ...
    def __init__(self, name, **kwargs):
        ...
        if self.element is not None:
            ...
            for i in range(len(childNodes)):
                if i < lastIndex:
                    childNodes[i].next_node = childNodes[i + 1]
                else:
                    pass
    ...
    def data(self, **kwargs):
        ...
        for adjacent_node in self.adjacents:
            ...
            elif adjacent_node is DOMElement and (max_depth is None or max_depth > 0):
                # recursively get the data from the data node.
                returned_data.append(adjacent_node.data(max_depth = max_depth))
        return ''.join(returned_data)
    ...
    def get(self, selector, **kwargs):
        ...
        for combinator in combinators:
            originalfound_elements.extend(self.__get(self, combinator, 0, checkOrigin = True, **kwargs))
        
        for foundElement in originalfound_elements:
            if foundElement in found_elements:
                continue
    ...
    def __get(self, DOMSubTree, selector, index, **kwargs):
        '''
        ...
        check_origin = 'checkOrigin' in kwargs and kwargs['checkOrigin']
        ...
        log.debug("DOMElement.__get: [INIT] Pointer:\t%s" % DOMSubTree)
        log.debug("DOMElement.__get: [INIT] Up:\t%s" % DOMSubTree.parent())
        log.debug("DOMElement.__get: [INIT] Next:\t%s" % DOMSubTree.next())
        log.debug("DOMElement.__get: [INIT] Prev:\t%s" % DOMSubTree.prev())
        log.debug("DOMElement.__get: [INIT] Use:\t%s" % selector[index])
        
        # Check if the root of this subtree is on the path
        if check_origin and self.__require_next_selector(selector[index], DOMSubTree):
            ...
            if selector[index].end_of_query:
                ...
                found_elements.append(DOMSubTree)
                if not selector[index].is_wildcard() or (selector[index].is_wildcard() and len(DOMSubTree.children()) == 0):
        ...
        if flag.look_for_descendants or flag.look_for_children:
            ...
            childNodes = DOMSubTree.children()
            if len(childNodes) == 0:
            ...
            if not self.__require_next_selector(selector[index], DOMSubTree) and next_node is None:
                log.debug("DOMElement.__get: Sibling > Level > Next")
                for node in DOMSubTree.children():
                    self.__add_target_or_continue(
                        node,
                        selector,
                        index,
            ...
            elif self.__require_next_selector(selector[index], DOMSubTree) and next_node is not None:
    ...
    def __add_target_or_continue(self, reference_node, selector, index, found_elements, **kwargs):
        ...
        if self.__require_next_selector(selector[index], reference_node):
            ...
            if add_reference_node:
                found_elements.append(reference_node)
            ...
            if proceed_to_next_rule:
                ...
                if flag.look_for_descendants:# and not force_stop:
                    ...
                    found_elements.extend(self.__get(reference_node, selector, index + 1))
                # CASE KTB__G2/KTB__ATOKI1_3: Else if the selector looks for children
                    ...
                    found_elements.extend(found_elements_from_iteration)
                # CASE KTB__ATOKI1_3: Else if this is sibling search but the next node is available
                    ...
                    while next_node is not None:
                        found_elements.extend(self.__get(
                            next_node,
                            selector,
                            index + 1,
                            checkOrigin = True
        ...
        elif not flag.look_for_children:
            log.debug("DOMElement.__add_target_or_continue: Negative > Not looking for children")
    ...
    def __require_next_selector(self, single_selector, reference_node = None):
        ...
        try:
            ...
            if base.isString(single_selector.name) and not single_selector.is_wildcard():
                assert single_selector.name == test_node.name
            ...
            for attr_name, attr_value in single_selector.attributes.iteritems():
                try:
                    ...
                    assert attr_name in test_node.attrs
                    if attr_value:
                        if flag_begin_with:
